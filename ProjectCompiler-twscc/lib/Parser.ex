...
def parse_function( token_list) do
  ...
  func={:function, b, [state]}##aqui se agregan los nodos
...
def parse_statement(token_list) do
  ...
  state={h, nil , [exp] }#definicion de la expresion statement
...
def parse_exp(token_list) do
  ...
  case List.first(token_list) do
    ...
    {:constant, 2} -> parse_constant(token_list);
...
def parse_constant(token_list) do
  {a, token_list} = parsear(token_list, {:constant, 2})
